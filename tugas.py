...
if __name__ == '__main__':
    ...
    inputloop = True
    while inputloop:
        arrivalt = int(input(f'Masukkan Arrival Time P{i}: '))
        burstt = random.randint(5,30)
        readyQ.enqueue([f'P{i}',burstt,arrivalt])
        readyQ.queue.sort(key = lambda a: a[2])
        i += 1
        loopcheck = input('Ingin tambah process lagi? (ketik n bila tidak)\n')
    ...
    avgWT = readyQ.waitingT()
    avgTT = readyQ.turnaroundT()
    ...
    while readyQ.isEmpty() == False:           
        ...
        while exec[1] > 0:
            ...
            print('\nProcess sedang dieksekusi')
            print(f'|{exec[0]}  {exec[1]}',end='|\n\n')
            for i in readyQ.queue:
                if i[2] == count:
                    readyQ.showQ.append(i)
            print('Process dalam Queue')
